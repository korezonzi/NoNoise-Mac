...
- `Sources/Core` — engine, no UI:
  ...
  - `AudioProcessing/VoiceChain` + `Biquad` + `Dynamics` — post-DSP "voice polish": unified chain `hp → shelves → presence → deEsser → comp → limiter` (polish stages gated by `voicePolishEnabled`; clarity stages gated by `clarity != .off`; limiter always runs while active), driven by `ClarityLevel` and the per-preset `VoiceChainSettings`.
